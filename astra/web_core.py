"""Astra HTTP core — the transport-neutral half of the web layer.

Every route, security behaviour and response shape lives here *exactly
once*. The two servers are thin adapters over this module:

    astra/web.py          stdlib http.server  (zero-dependency default)
    astra/web_fastapi.py  FastAPI + uvicorn   (optional extra)

That split is deliberate. The earlier FastAPI attempt in this repo was a
second, hand-written copy of the whole 900-line route surface, which meant
every behaviour had to be kept in sync in two places by hand; it was
reverted for that reason. Here the adapters own only socket/ASGI plumbing
and delegate *all* decisions to `WebApp.handle()`, so the two servers
cannot drift apart.

Contract each adapter must honour:

  1. Build a `Request` (method, raw path, headers, parsed body/fields/files,
     client ip, request id) and call `WebApp.handle()`.
  2. Encode the returned `Response` with `response_headers()` — never add,
     drop or reorder security headers locally.
  3. A `Response.stream` is an iterator of bytes (SSE); if it is set the
     adapter must not send a Content-Length.

Security defaults are unchanged from astra/web.py: optional operator auth
(ASTRA_TOKEN), per-IP rate limiting, request body cap, security headers +
CSP, X-Request-Id on every response, structured JSON errors without stack
traces, and every payload redacted through security.redact before egress.
"""
...
class Response:
    """One HTTP response, ready to be encoded by any adapter.

    `stream` wins over `body` when set: the adapter must omit
    Content-Length and write the frames as they are produced.
    """

    __slots__ = ("status", "body", "stream", "astream", "content_type",
                 "headers", "cache", "cors", "hardened")

    def __init__(self, status=200, body=b"", content_type=None, headers=None,
                 stream=None, astream=None, cache="no-store", cors=False,
                 hardened=True):
        self.status = status
    ...
    def has_stream(self) -> bool:
        return self.stream is not None or self.astream is not None
...
class AstraSite:
    ...
    This is the object the route table calls into; it is deliberately not a
    socket server so the FastAPI app can reuse it verbatim. `AstraServer`
    mixes it with `ThreadingHTTPServer`; `make_app()` passes it to the ASGI
    layer unchanged.
    """
...
class WebApp:
    ...
    def _static(self, rel: str, req: Request) -> Response:
        ...
        if not os.path.isfile(path):
            # http.server's send_error() used to render an HTML error page
            # here; a short plain-text body is the one deliberate difference
            # from the pre-split server (same status, no HTML template).
    ...
    def _sse(self, req: Request) -> Response:
        """Both drives of the same feed: the stdlib adapter iterates `stream`,
        an ASGI adapter awaits `astream`. Two objects, one generator function,
        so a 45s Live-tab connection can never be driven differently by the
        two servers — and an ASGI server never has to hold a worker thread
        for it (see sse_frames_async)."""
        return Response(
            200,
            content_type="text/event-stream; charset=utf-8",
            stream=sse_frames(self.site, req),
            astream=sse_frames_async(self.site, req),
            cache="no-cache",
            cors=True,
...
def sse_frames(site: AstraSite, req: Request):
    """Synchronous SSE drive — used by the stdlib (blocking) server, which
    writes each frame straight to the socket."""
    after = _sse_start_id(site, req)
    written, deadline = 0, time.time() + SSE_DEADLINE_S
    while time.time() < deadline:
        evs = site.events().since(after)
        if evs:
            for e in evs:
...
async def sse_frames_async(site: AstraSite, req: Request):
    """Asynchronous SSE drive — the same feed, the same frames.

    Why this exists rather than reusing the sync generator: Starlette drives a
    *sync* generator inside its shared anyio worker threadpool. An idle Live
    tab holds a connection open for up to 45s, so a handful of them would
    occupy the very threads that run agent turns — the pool is 40 by default,
    and every blocked turn is a hung request. Here the wait is `asyncio.sleep`
    and only the (short) SQLite read is handed to the default executor, so an
    idle feed costs no worker thread at all.
    """
