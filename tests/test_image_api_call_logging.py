...
class TestRouterImageApiCallLog(unittest.TestCase):
    ...
    def _router(self, *adapters):
        ...
        bus = _bus()
        router = AstraRouter(list(adapters), max_retries=0)
        router.attach_events(bus)
        return router, bus

    def _route(self, router):
        from astra.ai.router import RoutingRequest
        return router.route_request(RoutingRequest(
            task_type="image_generation",
            messages=[{"role": "user", "content": "akta cat photo banao"}],
            trace="req-9"))

    def test_router_image_call_logs_start_then_success_with_status(self):
        # The router path names the adapter by its Provider name ("cloudflare"),
        # not by the Gateway connection id.
        img = _FakeConn("cloudflare", "cloudflare", image_models=[FLUX])
        router, bus = self._router(img)
        rr = self._route(router)
        self.assertTrue(rr.ok, rr.error)

        started = _data(bus, "ai.started")
        done = _data(bus, "ai.completed")
        self.assertEqual(len(started), 1)
        self.assertEqual(len(done), 1)
        self.assertEqual((started[0]["provider"], started[0]["model"]),
                         ("cloudflare", FLUX))
        self.assertEqual((done[0]["provider"], done[0]["model"]),
                         ("cloudflare", FLUX))
        self.assertEqual(done[0]["status_code"], 200)
        self.assertIn("latency_ms", done[0])
        self.assertEqual(started[0]["op"], done[0]["op"])

    def test_router_image_failure_logs_status_and_safe_reason(self):
        err = ProviderError("cloudflare rate limit reached")
        err.code = 429
        img = _FakeConn("cloudflare", "cloudflare",
                        image_models=[FLUX, LIGHTNING], outcomes=[err])
        router, bus = self._router(img)
        rr = self._route(router)
        self.assertTrue(rr.ok, rr.error)

        failed = _data(bus, "ai.failed")
        self.assertEqual(len(failed), 1)
        d = failed[0]
        self.assertEqual((d["provider"], d["model"]), ("cloudflare", FLUX))
        self.assertEqual(d["status_code"], 429)
        self.assertEqual(d["reason"], "429 rate limit")
        self.assertTrue(d["terminal"])
        # The successful fallback call is its own logged api call.
        done = _data(bus, "ai.completed")
        self.assertEqual(done[0]["model"], LIGHTNING)
        self.assertEqual(done[0]["status_code"], 200)
        self.assertNotEqual(d["op"], done[0]["op"])

    def test_router_failure_category_prefers_the_real_http_status(self):
        from astra.ai.router import AstraRouter
        cat = AstraRouter._image_failure_category
        self.assertEqual(cat("cloudflare provider error", 502),
                         "provider error (502)")
        self.assertEqual(cat("boom", 429), "429 rate limit")
        self.assertEqual(cat("boom", 408), "timeout")
        self.assertEqual(cat("boom", 404), "model unavailable")
        self.assertEqual(cat("boom", 401),
                         "provider authentication/credential failure")
        self.assertEqual(cat("boom", 0), "boom")
