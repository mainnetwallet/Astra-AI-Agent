"""Real end-to-end: Agent -> ChatPipeline -> ToolRegistry -> Terminal ->
result -> AI -> Terminal again.

Only the model reply is scripted (there is no live model offline); the
Agent, ChatPipeline, ToolRegistry, Terminal, file tools, git and the
...
class RealRepoTaskE2E(unittest.TestCase):
    ...
    def test_fix_failing_tests_end_to_end(self):
        ...
        script = [
            ...
            _tool("terminal_exec", command=f"cd {self.repo} && python3 -B -m pytest -q -p no:cacheprovider"),
        ...
        self.assertEqual([s["tool"] for s in trace["steps"]],
                         ["terminal_exec", "read_file", "write_file",
        ...
        # 7: session + cwd stayed correct across calls
        session = manager.get("conv-42", create=False)
        self.assertIsNotNone(session)
        self.assertEqual(os.path.realpath(session.cwd),
        ...
        rerun = registry.execute("terminal_history", {}, ctx=registry_ctx(manager))
        cmds = [h["command"] for h in rerun["result"]["commands"]]
        ...
        self.assertEqual(registry.stats("terminal_exec")["calls"], 3)
        manager.close_all()
    ...
    def test_tool_loop_context_survives_multiple_calls(self):
        ...
        router = ScriptedRouter([
            _tool("terminal_exec", command="echo FIRST-MARK"),
            _tool("terminal_exec", command="echo SECOND-MARK"),
            _final("marks done"),
        ])
        ...
        session = manager.get("conv-9", create=False)
